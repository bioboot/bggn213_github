...
Absolutely-tootley the `winpercent` column is on a different "scale" or range than all the others.

**N.B** Wee will need to scale this data before analysis like PCA for example to avoid this one variable dominating our analysis.
...
Step 3. Calculate their mean `winpercent`

Step 4. Find/extract fruity candy
...
We can make this plot better by rearrange (with the `reorder()` function) the y-axis by winpercent so the highest scoring candy is at the top and lowest at the bottom.
