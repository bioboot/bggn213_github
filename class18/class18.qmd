...
## Background
Pertussis (a.k.a. wwhooping cough) is a serrious lung infection caused by the bacteria *B. Pertussis*.


## Tracking Pertussis cases
...
> Q2. Let's add the key milestones of DTP (wP) vaccine roll out in 1942 and the switch to the new DTaP (aP) vaccine in 1996. We can use `geom_vline()` for this. Booster shots started in 2004.
...
There were high case numbers pre 1946 (before the wP vaccine) then relatively rapid decrease in case numbers through the 1970s and on to 2004 when our first widespread outbreak occured again.
...
One of the main goals of this project is to determine what is different in the immune response between wP and aP primmed individuals.
...
head(ab_titer)
```
To use this data we need to "join" the various tables to find all the information I need to know about a particular measurment.
...
>Q. How many differen antibody isotypes are we measuring?
...
```{r}
today()  -  mdy("03-30-1997")
today() - dmy("22-06-1978")
```

```{r}
time_length( today()  -  mdy("03-30-1997"), "years")
#month-day-year
